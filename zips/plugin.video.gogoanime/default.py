...
def CATEGORIES():
        addDir('Anime Full List','http://www.gogoanime.com/free-anime-list',1,artwork + 'full_list.png')
        addDir('New Series','http://www.gogoanime.com/category/new-release',1,artwork + 'nseries.png')
...
def INDEXSHOWS(url):
        ...
        link=response.read()
        response.close()
        nextpage=re.compile('<span class="current">.+?</span><a href="(.+?)" class=".+?" title=".+?">.+?</a>').findall(link)
        if len(nextpage) > 0:
                addDir('Next Page',nextpage[0],1,artwork + 'next.png')
        if 'http://www.gogoanime.com/free-anime-list' in url:
        ...
        elif 'http://www.gogoanime.com/category/new-release' in url:
                ...
                for url,name,thumbnail in match:
                        ...
                        if "<span>" not in name:
                               addDir(name,showurl + str(url),2,thumbnail)
